...
export namespace GLAccountCommands {
  ...
  export type GetByNumber = (
    accountNumber: string
  ) => Promise<TauriResponse<GLAccount>>
  export type Create = (
  ...
  export type GetBalances = (
    asOfDate?: Date
  ) => Promise<TauriResponse<VAccountBalance[]>>
}
...
export namespace TokenCommands {
  ...
  export type GetBySymbol = (
    symbol: string,
    chainId?: string
  ) => Promise<TauriResponse<Token[]>>
  export type Create = (
...
export namespace WalletCommands {
  ...
  ) => Promise<TauriResponse<Wallet>>
  export type UpdateSyncStatus = (
    id: number,
    lastSynced: Date
  ) => Promise<TauriResponse<boolean>>
  export type Delete = (id: number) => Promise<TauriResponse<boolean>>
...
export namespace TransactionCommands {
  export type GetAll = (
    ...
    sort?: SortOptions
  ) => Promise<TauriResponse<PaginatedResponse<EnrichedAccountingTransaction>>>
  export type GetById = (
    id: number
  ) => Promise<TauriResponse<EnrichedAccountingTransaction>>
  export type GetByAccount = (
    accountId: number,
    dateRange?: DateRange
  ) => Promise<TauriResponse<AccountingTransaction[]>>
  export type GetByToken = (
    tokenId: number,
    dateRange?: DateRange
  ) => Promise<TauriResponse<AccountingTransaction[]>>
  export type GetByWallet = (
    walletAddress: string,
    chainId: string
  ) => Promise<TauriResponse<AccountingTransaction[]>>
  export type GetUnreconciled = () => Promise<
    TauriResponse<AccountingTransaction[]>
  >
  export type Create = (
    request: CreateAccountingTransactionRequest
  ) => Promise<TauriResponse<AccountingTransaction>>
  export type Update = (
    id: number,
    updates: Partial<AccountingTransaction>
  ) => Promise<TauriResponse<AccountingTransaction>>
  export type Delete = (id: number) => Promise<TauriResponse<boolean>>
...
export namespace JournalEntryCommands {
  export type GetAll = (
    filters?: FilterOptions
  ) => Promise<TauriResponse<EnrichedJournalEntry[]>>
  export type GetById = (
    id: number
  ) => Promise<TauriResponse<EnrichedJournalEntry>>
  export type GetByDateRange = (
    dateRange: DateRange
  ) => Promise<TauriResponse<JournalEntry[]>>
  ...
  export type Validate = (
    id: number
  ) => Promise<TauriResponse<{ isValid: boolean; errors: string[] }>>
}
...
export namespace LotCommands {
  ...
  export type GetByToken = (
    tokenId: number
  ) => Promise<TauriResponse<TransactionLot[]>>
  export type GetOpen = (
    tokenId?: number
  ) => Promise<TauriResponse<VOpenTaxLot[]>>
  export type GetById = (id: number) => Promise<TauriResponse<TransactionLot>>
  export type RecordDisposal = (
    request: RecordDisposalRequest
  ) => Promise<TauriResponse<boolean>>
  export type GetDisposals = (lotId: number) => Promise<TauriResponse<any[]>>
  export type GetRealizedGains = (
    taxYear?: number
  ) => Promise<TauriResponse<VGainsLossesSummary[]>>
}
...
export namespace PriceCommands {
  export type GetLatest = (
    tokenId: number
  ) => Promise<TauriResponse<PriceHistory>>
  ...
  export type BulkUpdatePrices = (
    updates: UpdatePriceRequest[]
  ) => Promise<TauriResponse<number>>
  export type FetchFromCoingecko = (
    tokenIds: number[]
  ) => Promise<TauriResponse<number>>
}
...
export namespace ReportingCommands {
  export type GetBalanceSheet = (
    asOfDate: Date
  ) => Promise<TauriResponse<BalanceSheetReport>>
  export type GetIncomeStatement = (
    dateRange: DateRange
  ) => Promise<TauriResponse<IncomeStatementReport>>
  export type GetCashFlow = (
    dateRange: DateRange
  ) => Promise<TauriResponse<CashFlowStatement>>
  export type GetTrialBalance = (
    asOfDate: Date
  ) => Promise<TauriResponse<TrialBalance>>
  export type GetPortfolio = (
    asOfDate: Date
  ) => Promise<TauriResponse<PortfolioReport>>
  export type GetTaxReport = (
    taxYear: number
  ) => Promise<TauriResponse<TaxReport>>
  export type GetDashboard = () => Promise<TauriResponse<DashboardData>>
  export type GetGeneralLedger = (
    filters: FilterOptions
  ) => Promise<TauriResponse<VGeneralLedgerEntry[]>>
  export type GetTaxSummary = (
    taxYear: number
  ) => Promise<TauriResponse<VTaxSummary>>
}
...
export namespace CustomReportCommands {
  ...
  export type GetById = (
    id: number
  ) => Promise<TauriResponse<CustomReportDefinition>>
  export type Create = (
  ...
  export type Execute = (
    id: number
  ) => Promise<TauriResponse<CustomReportResult>>
}
...
export namespace ExportCommands {
  ...
  ) => Promise<TauriResponse<ExportResult>>
  export type ExportTransactions = (
    filters: FilterOptions,
    format: 'csv' | 'excel'
  ) => Promise<TauriResponse<ExportResult>>
  export type ExportTaxForm = (
    taxYear: number,
    form: '8949' | 'schedule_d'
  ) => Promise<TauriResponse<ExportResult>>
}
...
export namespace ReconciliationCommands {
  export type GetReconciliationReport = (
    accountId: number,
    tokenId: number,
    chainId: string
  ) => Promise<TauriResponse<ReconciliationReport>>
  export type MatchTransaction = (
    accountingTxnId: number,
    blockchainTxnHash: string
  ) => Promise<TauriResponse<boolean>>
  export type UnmatchTransaction = (
    accountingTxnId: number
  ) => Promise<TauriResponse<boolean>>
  export type AutoReconcile = (
    accountId: number
  ) => Promise<TauriResponse<{ matched: number; unmatched: number }>>
}
...
export namespace PortfolioCommands {
  ...
  export type GetTokenBalances = (
    accountId?: number
  ) => Promise<TauriResponse<VTokenBalance[]>>
  export type GetPerformance = (
    dateRange: DateRange
  ) => Promise<TauriResponse<PerformanceData>>
  export type GetAllocation = () => Promise<
    TauriResponse<AllocationChartData[]>
  >
}
...
export namespace AnalyticsCommands {
  export type GetPortfolioValueHistory = (
    dateRange: DateRange
  ) => Promise<TauriResponse<PortfolioValuePoint[]>>
  export type GetTransactionVolume = (
    dateRange: DateRange
  ) => Promise<TauriResponse<TransactionVolumePoint[]>>
  export type GetGainsLossesChart = (
    dateRange: DateRange
  ) => Promise<TauriResponse<GainsLossesChartData[]>>
  export type GetAllocationChart = (
    groupBy: 'asset_type' | 'chain' | 'account'
  ) => Promise<TauriResponse<AllocationChartData[]>>
}
...
export namespace PreferenceCommands {
  ...
  export type SetDefaultCostBasisMethod = (
    method: string
  ) => Promise<TauriResponse<boolean>>
  export type SetReportingCurrency = (
    currency: string
  ) => Promise<TauriResponse<boolean>>
}
...
export namespace DatabaseCommands {
  export type RunMigrations = () => Promise<
    TauriResponse<{ migrationsRun: number }>
  >
  export type Backup = (
    path: string
  ) => Promise<TauriResponse<{ backupPath: string; size: number }>>
  ...
  export type ValidateIntegrity = () => Promise<
    TauriResponse<{ isValid: boolean; issues: string[] }>
  >
}
...
export namespace BatchCommands {
  export type ImportTransactions = (
    transactions: CreateAccountingTransactionRequest[]
  ) => Promise<
    TauriResponse<{ imported: number; failed: number; errors: string[] }>
  >
  export type ImportPrices = (
    prices: UpdatePriceRequest[]
  ) => Promise<TauriResponse<{ imported: number; failed: number }>>
  export type BulkCreateJournalEntries = (
    entries: CreateJournalEntryRequest[]
  ) => Promise<TauriResponse<{ created: number; failed: number }>>
}
