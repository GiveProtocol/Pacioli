...
const WalletManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      console.log(
        `💾 loadData called: dbInitialized=${dbInitialized}, selectedAddress=${selectedAddress}, selectedNetwork=${selectedNetwork}`
      )

      if (!dbInitialized || !selectedAddress) {
        console.log(
          `💾 Early return: dbInitialized=${dbInitialized}, selectedAddress=${selectedAddress}`
        )
        setSyncStatus(null)
        setTransactions([])
        return
      ...
      try {
        ...
        console.log(
          `💾 Is EVM: ${isEVMAddress}, Network address: ${networkAddress}`
        )
        ...
        console.log(
          `💾 Setting transactions state with ${savedTxs.length} transactions for ${networkAddress}`
        )
  ...
  const handleWalletsChange = useCallback(
    async (wallets: ConnectedWallet[]) => {
      ...
      console.log(
        `💾 handleWalletsChange: selectedAddress='${selectedAddress}', lastSavedAddress='${lastSavedAddress}'`
      )
      if (
        wallets.length > 0 &&
        ...
        console.log(
          `💾 Auto-selecting first address: ${wallets[0].accounts[0].address}`
        )
